...
app.get("/authorize", async (c) => {
	...
	return renderApprovalDialog(c.req.raw, {
		...
		csrfToken,
		server: {
			description: "Remote MCP server for Workflowy, backed by a D1 mirror.",
 ...
 * It exchanges the temporary code for an access token, checks the GitHub
 * username against the ALLOWED_GITHUB_USERS allowlist, then stores some
...
app.get("/callback", async (c) => {
	...
	// Reject users not present in the ALLOWED_GITHUB_USERS allowlist
	const allowedUsers = getAllowedUsers();
	if (!allowedUsers.has(login)) {
		return c.text(`Access denied: GitHub user "${login}" is not on the allowlist.`, 403);
	}
	...
	const { redirectTo } = await c.env.OAUTH_PROVIDER.completeAuthorization({
		...
		// This will be available on this.props inside WorkflowyMCP
		props: {
			accessToken,
			email,
			login,
